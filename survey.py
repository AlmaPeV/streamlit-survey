...
def conduct_survey():
    ...
    if round_number <= 9:
        ...
        if st.button("Next Round", key=f"next_button_{round_number}"):
            ...
            # Subir los resultados a Firestore
...
if st.session_state.authenticated and not st.session_state.survey_completed:
    ...
    for survey in surveys:
        ...
        st.write(f"Participant: {survey_data['participant_name']}, Round {survey_data['round']}: Appeared Samples: {survey_data['appeared_samples']}, Selected Sample: {survey_data['selected_sample']}")

# Mostrar muestras actuales del panelista en tiempo real durante la encuesta
if st.session_state.authenticated and st.session_state.survey_started and not st.session_state.survey_completed:
